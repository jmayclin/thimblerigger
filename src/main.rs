...
fn main() {
    ...
    for file in files {
        ...
        for line in fs::read_to_string(file).unwrap().lines() {
            ...
            let result = thimblerigger::solve(board, & mut table);
            if expect != result {
                println!("Your incompetence is astounding. State: {} was {} but should be {}", state, result, expect);
            }
        }
        println!("{} | average time of {} microseconds from {} trials", file, now.elapsed().as_micros() / count, count);
    }
}
